...
if [ ! -f genome_list.txt ]; then
	find $genome_file_in -type f -name "*.fasta" -maxdepth 1 | sort > genome_list.txt
fi
...
hisat2 -p 32 -x "${genome_file_in##*/}_index" -1 ${fastq_PE_1} -2 ${fastq_PE_2} -S "${genome_file_in##*/}_PE_Hisat2.sam"
