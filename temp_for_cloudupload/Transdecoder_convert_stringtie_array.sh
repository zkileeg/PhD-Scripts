...
#set log variable so we can output steps as we go. THis was included originally during troubleshooting stages. 
log_out=transdecoder_pipe_log_${SLURM_ARRAY_TASK_ID}.txt

#wait before starting to ensure the first array number runs first so we can get the file lists
...
#assigning some variables for output
