...
source("Network_Functions.R") # Contains some self-defined functions for customization
...
#Use custom function from generic to edit the df title
base = name_edit(base, base = TRUE) 

#Use custom function to map gene nicknames to ATG names 
base = mapper(base, map)  

#Constructing Interaction Network using igraph
g = graph_from_data_frame(base, directed = FALSE, vertices = NULL)  

#Creating separate dataframes for tissues by averaging over biological replicates in Dataset (2) {High Correlation observed}
#Root specific df with expression set to the mean Travadb expression for each gene in the whole root replicates
a_r = data.frame(Gene = df$Gene, Value = (df$`Root (R )...7`+df$`Root (R )...76`)/2)
#Same for mature leaf
a_l = data.frame(Gene = df$Gene, Value = (df$`Leaf, mature (L.lg)...18` + df$`Leaf, mature (L.lg)...87`)/2)

#Now build tissue specific sub-graphs for each tissue, using only nodes with expression >1
sg_r = rest_subgraph(g, base, a_r, 1, T) 
sg_l = rest_subgraph(g, base, a_l, 1, T)
```
...
#calculate the cluster using walktrap on graph g and 8 steps
cg = cluster_walktrap(g, steps = 8)

#calculate the pagerank for each node
prg = page_rank(g, directed = F)

#Custom vis function for plotting the network
...
d = k_list(g,conf,1) 
#Highlight retreived genes in the network
...
```{r}
#calculate the authority score for each node in the subgraph in root
auth_r = authority_score(sg_r)
#create a df that has just the gene name and associated authority score
auth_r = data_frame(Gene = names(auth_r$vector), Value = auth_r$vector)
#check each node name in the subgraph and return TRUE if the authority score of that node is >0.5
comm = V(sg_r)$name %in% auth_r$Gene[auth_r$Value>0.5]
#Turn F and T into 1 and 2
comm = comm + 1
#Use those values as the community memberships in the network plot so that nodes with authority scores >0.5 are in group 2
...
diff = diff_plot(sg_r, sg_l) 

#Custom function that produces a vertex list for the differences where in this case all verrtices unique to the leaf will be highlighted
comm = high_nodes(diff, V(sg_l)$name[!(V(sg_l)$name %in% V(sg_r)$name)])

#Custom ploting function to show difference network with node and edge colour based on presense/absense in networks
#Edges coloured black are ones in Network #1 and not in Network #2 and the edges coloured red are edges in #2 but not in #1.
#Nodes in network #2 but not in Network #1 are highlighted
...
```{r}
#read in the excel table with the inefction expression data
df = read_excel("infection_data.xlsx", sheet = 2)
#Average the avlues from the biological replicates (these show strong correlation)
...
#Assemble all of these average values into an input df called av_df
...
```{r}
#Plot the newtork limited to genes expressed after 1 hour of infection (Highlight nodes that are expressed at TS 1 but not at TS 6)
#Build a df containing just the expression data at 1 hour of infection
temp_df1 = data_frame(Gene = av_df$Gene, Value = av_df$vir_1)
#Now build specific sub-graph using only nodes with expression >1 at vir_1 time point 
v_1 = rest_subgraph(g, base, temp_df1, 1)
#Create a list of genes that are expressed at vir_1, but not at vir_6
tlist_1_6 = intersect(V(v_1)$name, av_df$Gene[(av_df$vir_1>=1) & (av_df$vir_6<1)])
#Plot the subgraph and highlight nodes that turn off at 6 hours using PR to map node size 
...
```{r}
#Analysis that highlights nodes with high pagerank values
Page_r = data_frame(Gene = names(pr_r$vector), Value = pr_r$vector)
comm = V(sg_r)$name %in% Page_r$Gene[Page_r$Value>0.02]
comm = comm + 1
visPlot(sg_r, comm, pr_r$vector)
```
